...
export function AIResponseConfigForm({
  initial,
  onSave,
}: {
  ...
  const removeFromList = (list: "whitelist" | "blacklist", index: number) => {
    update(
      list,
      config[list].filter((_, i) => i !== index)
    );
  };
  ...
  const Toggle = ({
    ...
    <label className="flex items-start gap-3 py-2.5 cursor-pointer group">
      ...
      >
        <span
          className={`absolute top-0.5 left-0.5 h-4 w-4 bg-white rounded-full shadow transition-transform ${value ? "translate-x-5" : ""}`}
        />
      </button>
    </label>
  );
  ...
  const Section = ({
    ...
    <div className="border border-slate-100 rounded-xl overflow-hidden">
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 text-left ${expanded === id ? "bg-slate-50" : "hover:bg-slate-25"}`}
      >
  ...
  return (
    <div className="space-y-4">
      <Section
        id="response"
        icon={MessageCircle}
        title="Configuracoes de Resposta"
        color="text-green-600"
      >
        <Toggle
          value={config.keepUnread}
          onChange={(v) => update("keepUnread", v)}
          label="Manter nao lida ao responder"
          desc="Marca a conversa como nao lida apos IA responder"
        />
        <Toggle
          value={config.singleMessage}
          onChange={(v) => update("singleMessage", v)}
          label="Enviar resposta em uma mensagem"
          desc="Consolida a resposta da IA em 1 unica mensagem"
        />
        <Toggle
          value={config.includeContactName}
          onChange={(v) => update("includeContactName", v)}
          label="Incluir nome do contato"
          desc="A IA usa o nome do lead na resposta"
        />
        <Toggle
          value={config.cancelOnNewMsg}
          onChange={(v) => update("cancelOnNewMsg", v)}
          label="Cancelar ao receber nova mensagem"
          desc="Se o lead enviar outra msg enquanto IA processa, cancela a anterior"
        />
        <Toggle
          value={config.pauseAfterManual}
          onChange={(v) => update("pauseAfterManual", v)}
          label="Nao responder apos mensagem manual"
          desc="Pausa a IA quando voce responde manualmente"
        />
        <div className="py-2">
          ...
          <span className="text-xs text-slate-400 block mt-0.5 mb-1.5">
            Enviada quando receber audio, sticker, etc
          </span>
          <input
            type="text"
      ...
      <Section id="timers" icon={Clock} title="Tempo e Humanizacao" color="text-amber-600">
        <NumberInput
          value={config.waitBeforeReply}
          onChange={(v) => update("waitBeforeReply", v)}
          label="Aguardar antes de responder"
          desc="Tempo de espera antes de processar a mensagem"
          min={0}
          max={30}
          suffix="segundos"
        />
        <NumberInput
          ...
          onChange={(v) => update("delayPerChar", v)}
          label="Delay por caractere"
          desc="Simula tempo de digitacao"
          min={0}
          max={500}
          suffix="ms"
        />
        <NumberInput
          ...
          onChange={(v) => update("delayMax", v)}
          label="Delay maximo"
          desc="Limite do tempo de digitacao"
          min={0}
          max={30000}
          suffix="ms"
        />
      ...
      <Section id="human" icon={Shield} title="Intervencao Humana" color="text-blue-600">
        <Toggle
          value={config.humanIntervention}
          onChange={(v) => update("humanIntervention", v)}
          label="Ativar intervencao humana"
          desc="Pausa a IA automaticamente quando voce responde manualmente"
        />
        <NumberInput
          value={config.humanPauseHours}
          onChange={(v) => update("humanPauseHours", v)}
          label="Reativacao"
          desc="Horas para reativar a IA apos intervencao"
          min={1}
          max={72}
          suffix="horas"
        />
      ...
      <Section id="filters" icon={Filter} title="Filtros de Numero" color="text-red-600">
        <Toggle
          value={config.ignoreGroups}
          onChange={(v) => update("ignoreGroups", v)}
          label="Ignorar grupos"
          desc="A IA nao responde em grupos do WhatsApp"
        />

        <div className="py-3">
          <label className="text-sm text-slate-700 font-medium block mb-2">
            Somente Responder (whitelist)
          </label>
          ...
          <div className="flex flex-wrap gap-1.5">
            {config.whitelist.map((num, i) => (
              <span
                key={i}
                className="flex items-center gap-1 bg-green-50 text-green-700 text-xs px-2 py-1 rounded-lg"
              >
                {num}{" "}
                <button onClick={() => removeFromList("whitelist", i)}>
                  <X className="h-3 w-3" />
                </button>
              </span>
            ))}
            {config.whitelist.length === 0 && (
              <span className="text-xs text-slate-400">Vazio = responde todos</span>
            )}
          </div>
        </div>
        ...
        <div className="py-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {config.blacklist.map((num, i) => (
              <span
                key={i}
                className="flex items-center gap-1 bg-red-50 text-red-700 text-xs px-2 py-1 rounded-lg"
              >
                {num}{" "}
                <button onClick={() => removeFromList("blacklist", i)}>
                  <X className="h-3 w-3" />
                </button>
              </span>
            ))}
            {config.blacklist.length === 0 && (
              <span className="text-xs text-slate-400">Nenhum numero bloqueado</span>
            )}
          </div>
        </div>
      </Section>
      ...
      <Section id="followup" icon={RefreshCw} title="Follow-Up Automatico" color="text-violet-600">
        <Toggle
          ...
          desc="Envia mensagens automaticas para leads sem resposta"
        />
        {config.followUpEnabled && (
          <>
            <Toggle
              value={config.followUpUseAI}
              onChange={(v) => update("followUpUseAI", v)}
              label="Gerar mensagens por IA"
              desc="A IA cria mensagens personalizadas de follow-up"
            />
            <NumberInput
              value={config.followUpMessages}
              onChange={(v) => update("followUpMessages", v)}
              label="Quantidade de mensagens"
              min={1}
              max={5}
            />
            <NumberInput
              value={config.followUpCheckMins}
              onChange={(v) => update("followUpCheckMins", v)}
            ...
            />
            <NumberInput
              value={config.followUpIntervalH}
              onChange={(v) => update("followUpIntervalH", v)}
            ...
            />
            <Toggle
              value={config.followUpRespectBH}
              onChange={(v) => update("followUpRespectBH", v)}
              label="Respeitar horario comercial"
              desc="So envia durante horario comercial (8h-18h)"
            />
          </>
        )}
      </Section>
      ...
      <Section id="audio" icon={Volume2} title="Audio com IA" color="text-emerald-600">
        <Toggle
          value={config.audioAutoReply}
          onChange={(v) => update("audioAutoReply", v)}
          label="Resposta automatica com audio"
          desc="Quando receber audio, responde com audio"
        />
        <Toggle
          value={config.audioReplaceText}
          onChange={(v) => update("audioReplaceText", v)}
          label="Substituir texto por audio"
          desc="Envia apenas audio (sem texto)"
        />
        <div className="py-2">
        ...
        <NumberInput
          ...
          onChange={(v) => update("audioMinChars", v)}
          label="Minimo de caracteres para audio"
          desc="Respostas menores nao geram audio"
          min={20}
          max={500}
          suffix="caracteres"
        />
