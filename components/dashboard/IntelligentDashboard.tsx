...
export function IntelligentDashboard({
  ...
  data: { alerts: DashboardAlert[]; kpis: DashboardKPI; agenda: AgendaItem[] };
}) {
        ...
        <div className="space-y-2">
          {alerts.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className={`border rounded-xl p-3 flex items-start gap-3 ${style.bg}`}
              >
                ...
                {alert.action && (
                  <Link
                    ...
                    className="shrink-0 flex items-center gap-1 px-3 py-1.5 bg-white rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 border border-slate-200 shadow-sm"
                  >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <Link
            key={kpi.label}
            href={kpi.href}
            className="bg-white border border-slate-100 rounded-xl p-3 hover:shadow-sm transition-all group"
          >
