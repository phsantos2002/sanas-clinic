...
export async function SystemStatus() {
  const [waConfig, aiConfig] = await Promise.all([getWhatsAppConfig(), getAIConfig()]);
  ...
  const label =
    hasWhatsApp && hasAI ? "Online" : hasWhatsApp ? "WhatsApp" : hasAI ? "IA ativa" : "Configurar";

  return (
    <div
      className={`flex items-center gap-1.5 text-xs ${isOnline ? "text-emerald-600" : "text-slate-400"}`}
    >
