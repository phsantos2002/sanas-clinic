...
export function DashboardClient({ leads, columns, stats, stages, tags }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Filter bar */}
      <div className="flex items-center gap-2 flex-wrap bg-white border border-zinc-200 rounded-lg px-3 py-2">
        {/* Search */}
        ...
        <select
          ...
          className="h-8 text-xs border border-zinc-200 rounded-md px-2 bg-white text-zinc-700 outline-none focus:ring-1 focus:ring-zinc-300"
        >
        ...
        <div className="relative">
          ...
          {showSavedDropdown && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-zinc-200 rounded-lg shadow-lg min-w-[200px] py-1">
                {savedFilters.length === 0 ? (
                  <p className="text-xs text-zinc-400 px-3 py-2">Nenhum filtro salvo</p>
                ) : (
                  savedFilters.map((f, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between px-3 py-1.5 hover:bg-zinc-50 cursor-pointer"
                    >
                      <button
                        onClick={() => applySavedFilter(f)}
                        className="text-xs text-zinc-700 flex-1 text-left"
                      >
                        {f.name}
                      </button>
                      <button
                ...
                <div className="border-t border-zinc-100 mt-1 pt-1">
                  <button
                    ...
                    className="w-full text-left text-xs px-3 py-1.5 text-blue-600 hover:bg-blue-50 disabled:text-zinc-300 disabled:hover:bg-transparent"
                  >
                    + Salvar filtro atual
                  </button>
                </div>
        ...
        {/* View toggle */}
        <div className="flex items-center gap-0.5 border border-zinc-200 rounded-md p-0.5 ml-auto">
          <button
