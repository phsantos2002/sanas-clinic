...
import { updateAdSetBidCap, getMetaAds, type MetaAdSet, type MetaAd } from "@/app/actions/meta";
...
export function AdSetItem({ adSet, campaignCpc, campaignCpm, benchmark }: Props) {
  ...
  const warningCount = ads.filter((a) =>
    ["saturating", "declining"].includes(getCreativeHealth(a))
  ).length;

  return (
    <div
      className={`bg-white border rounded-xl overflow-hidden ${isActive ? "border-blue-100" : "border-slate-100"}`}
    >
      <div className="p-4">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded-md font-medium ${isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                >
                ...
                {ads.length > 0 && (
                  <span className="text-[10px] text-slate-400">
                    ...
                    {warningCount > 0 && (
                      <span className="text-amber-500 ml-1">({warningCount} atenção)</span>
                    )}
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            {!editBidCap ? (
              ...
              <div className="flex items-center gap-1.5">
                ...
                <Button
                  size="sm"
                  onClick={handleSaveBidCap}
                  disabled={isPending}
                  className="h-6 text-[10px] rounded-md px-2"
                >
                  OK
                </Button>
                <button onClick={() => setEditBidCap(false)} className="text-[10px] text-slate-400">
                  &#10005;
                </button>
              </div>
            )}
        ...
        {editBidCap && suggestedBidMin != null && suggestedBidMax != null && (
          <div className="mt-2 ml-6 flex items-start gap-1.5 text-[10px] text-blue-600 bg-blue-50 rounded-lg px-2.5 py-1.5">
            ...
            <span>
              ...
              <span className="font-bold">{fmtBrl(suggestedBidMax)}</span>. Valores abaixo podem
              limitar entrega; acima aumentam custo sem ganho proporcional.
            </span>
          </div>
        )}
            ...
            if (ratio < 0.7)
              return (
                <div className="mt-2 ml-6 flex items-start gap-1.5 text-[10px] text-red-600 bg-red-50 rounded-lg px-2.5 py-1.5">
                  ...
                  <span>
                    Bid Cap ({fmtBrl(adSet.bidAmount!)}) está{" "}
                    <span className="font-bold">{Math.round((1 - ratio) * 100)}% abaixo</span> do
                    CPC real ({fmtBrl(campaignCpc)}). A entrega pode estar sendo limitada. Considere
                    aumentar para pelo menos {fmtBrl(campaignCpc * 0.8)}.
                  </span>
                </div>
              );
            if (ratio > 2)
              return (
                <div className="mt-2 ml-6 flex items-start gap-1.5 text-[10px] text-amber-600 bg-amber-50 rounded-lg px-2.5 py-1.5">
                  ...
                  <span>
                    Bid Cap ({fmtBrl(adSet.bidAmount!)}) está{" "}
                    <span className="font-bold">{Math.round((ratio - 1) * 100)}% acima</span> do CPC
                    real ({fmtBrl(campaignCpc)}). Você pode estar pagando mais do que necessário.
                    Considere reduzir para {fmtBrl(campaignCpc * 1.3)}.
                  </span>
                </div>
              );
            return (
        ...
        <div className="border-t border-slate-50 bg-slate-50/30 px-4 py-3 space-y-3">
          ...
          {loadingAds ? (
            ...
            <div className="space-y-2">
              {ads.map((ad) => (
                <AdCard key={ad.id} ad={ad} benchmark={benchmark} />
              ))}
                ...
                <div className="flex items-center gap-3 mt-2 pt-2 border-t border-slate-100">
                  ...
                  {performingCount > 0 && (
                    <span className="text-[10px] text-emerald-600">
                      {performingCount} performando
                    </span>
                  )}
                  {warningCount > 0 && (
                    <span className="text-[10px] text-amber-600">
                      {warningCount} precisam de atenção
                    </span>
                  )}
                </div>
              )}
