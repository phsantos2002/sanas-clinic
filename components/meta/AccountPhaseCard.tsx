...
const PHASE_CONFIG: Record<Phase, { label: string; color: string; bg: string; border: string; icon: typeof BookOpen; tooltip: string }> = {
  LEARNING:    { label: "Aprendizado",   color: "text-blue-700",    bg: "bg-blue-50",    border: "border-blue-200", icon: BookOpen,   tooltip: "A Meta está otimizando a entrega. Evite mudanças por 3-5 dias." },
  STABILIZING: { label: "Estabilização", color: "text-amber-700",   bg: "bg-amber-50",   border: "border-amber-200", icon: Zap,       tooltip: "A conta está acumulando dados. Bom momento para testar Cost Cap." },
  SCALING:     { label: "Escala",        color: "text-emerald-700", bg: "bg-emerald-50", border: "border-emerald-200", icon: TrendingUp, tooltip: "Conta estável com histórico. Ideal para escalar com Bid Cap." },
  MATURE:      { label: "Maturidade",    color: "text-violet-700",  bg: "bg-violet-50",  border: "border-violet-200", icon: Crown,     tooltip: "Conta madura com dados robustos. Teste ROAS Mínimo para maximizar retorno." },
};
  ...
  if (!result) {
    return (
      <div className="flex items-center justify-between px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50">
        ...
        <Button size="sm" variant="ghost" onClick={handleDiagnose} disabled={isPending} className="h-7 text-[10px] rounded-lg gap-1">
    ...
    <div className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border ${cfg.border} ${cfg.bg}`}>
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0 flex-wrap">
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 top-full left-0 mt-1 w-64 p-2 rounded-lg bg-slate-900 text-white text-[10px] leading-relaxed shadow-lg">
              {cfg.tooltip}
              {result.reason && <><br /><br />{result.reason}</>}
            </div>
          )}
        </div>
        ...
        <span className="text-[10px] text-slate-600">
          Estratégia recomendada: <strong>{BID_LABELS[result.suggestedBidStrategy] ?? result.suggestedBidStrategy}</strong>
        </span>
      </div>
