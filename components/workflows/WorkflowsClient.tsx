...
import {
  ...
  type WorkflowData, type TriggerConfig, type StepData,
...
const TRIGGER_LABELS: Record<string, { label: string; icon: typeof Zap; color: string }> = {
  ...
  stage_change: { label: "Mudanca de Estagio", icon: ArrowRight, color: "bg-green-100 text-green-700" },
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      {workflows.length === 0 ? (
        <div className="bg-white border border-slate-100 rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-slate-400">Crie sua primeira automacao para automatizar tarefas</p>
        </div>
      ) : (
              ...
              <div key={wf.id} className="bg-white border border-slate-100 rounded-2xl p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${trigger.color}`}>
                      <TriggerIcon className="h-5 w-5" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-slate-900 text-sm">{wf.name}</h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${trigger.color}`}>
                          {trigger.label}
                        </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`h-8 w-8 rounded-lg flex items-center justify-center transition-colors ${
                        wf.isActive ? "bg-green-50 text-green-600 hover:bg-green-100" : "bg-slate-50 text-slate-400 hover:bg-slate-100"
                      }`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              {/* Trigger */}
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">Quando (trigger)</label>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(TRIGGER_LABELS).map(([key, cfg]) => {
                    const Icon = cfg.icon;
                    return (
                      <button key={key} onClick={() => setTriggerType(key)}
                        className={`flex items-center gap-2 p-3 rounded-xl text-xs font-medium transition-all ${
                          triggerType === key ? `${cfg.color} border-2 border-current` : "bg-slate-50 text-slate-500 border-2 border-transparent"
                        }`}>
                ...
                {triggerType === "inactivity" && (
                  <input type="number" value={triggerDays} onChange={(e) => setTriggerDays(Number(e.target.value))}
                    className="w-full mt-2 border border-slate-200 rounded-xl px-3 py-2 text-sm" placeholder="Dias de inatividade" />
                )}
                {triggerType === "tag_added" && (
                  <input type="text" value={triggerTag} onChange={(e) => setTriggerTag(e.target.value)}
                    className="w-full mt-2 border border-slate-200 rounded-xl px-3 py-2 text-sm" placeholder="Nome da tag" />
                )}
              </div>

              {/* Action */}
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">Entao (acao)</label>
                ...
                {actionType === "send_whatsapp" && (
                  <textarea value={actionMessage} onChange={(e) => setActionMessage(e.target.value)}
                    ...
                    rows={3} className="w-full mt-2 border border-slate-200 rounded-xl px-3 py-2 text-sm resize-none" />
                )}
                ...
                {(actionType === "add_tag" || actionType === "remove_tag") && (
                  <input type="text" value={actionTag} onChange={(e) => setActionTag(e.target.value)}
                    className="w-full mt-2 border border-slate-200 rounded-xl px-3 py-2 text-sm" placeholder="Nome da tag" />
                )}
              </div>
            </div>
            <div className="flex gap-3 p-5 border-t border-slate-100">
              <button onClick={() => setShowCreate(false)} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50">Cancelar</button>
