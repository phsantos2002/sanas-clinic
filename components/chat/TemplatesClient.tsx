...
export function TemplatesClient({ templates }: { templates: TemplateData[] }) {
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <p className="text-xs text-slate-400">Use {'{{nome}}'} e {'{{clinica}}'} como placeholders. Atalhos como /oi funcionam no chat.</p>
      ...
      {templates.length === 0 ? (
        ...
        <div className="space-y-2">
          {templates.map((tpl) => (
            <div key={tpl.id} className="bg-white border border-slate-100 rounded-xl p-4 flex items-start gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h4 className="font-medium text-slate-900 text-sm">{tpl.name}</h4>
                  <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{tpl.category}</span>
                  {tpl.shortcut && (
                    <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full flex items-center gap-0.5">
                      <Hash className="h-2.5 w-2.5" />{tpl.shortcut}
                    </span>
                  )}
              ...
              <button onClick={() => handleDelete(tpl.id)} className="h-7 w-7 rounded-lg hover:bg-red-50 flex items-center justify-center">
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-5 shadow-2xl space-y-4">
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome (ex: Saudacao)"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <input type="text" value={shortcut} onChange={(e) => setShortcut(e.target.value)} placeholder="Atalho (ex: /oi)"
