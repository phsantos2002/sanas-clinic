...
export function ChatPageClient() {
  ...
  return (
    <div className="flex h-[calc(100vh-7.5rem)] -mx-4 md:-mx-6 overflow-hidden rounded-xl border border-slate-200">
      ...
      {/* ─── Chat area ─── */}
      <div
        className={`${showMobileSidebar ? "hidden" : "flex"} md:flex flex-1 flex-col overflow-hidden min-w-0`}
      >
        {selectedChat ? (
          <>
            {/* Chat header */}
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-white border-b border-slate-100">
                ...
                <button
                  onClick={() => {
                    ...
                    setMsgSearch("");
                    if (selectedChat) fetchMessages(selectedChat.wa_chatid);
                  }}
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4 bg-slate-50">
            ...
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-2 mt-3">
                <span
                  className={`w-2 h-2 rounded-full ${connectionStatus === "connected" ? "bg-emerald-500" : "bg-red-500"}`}
                />
      ...
      {forwardingMsg && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-2xl w-full max-w-sm max-h-[60vh] flex flex-col"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-[10px] flex-shrink-0 ml-2 ${chat.wa_unreadCount > 0 ? "text-emerald-600 font-semibold" : "text-slate-400"}`}
            >
              {time}
            </span>
          </div>
        ...
        <button
          onClick={(e) => {
            e.stopPropagation();
            onMenuToggle(showMenu ? null : chat.wa_chatid);
          }}
