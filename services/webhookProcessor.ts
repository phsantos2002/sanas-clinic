...
// Per-inbound-message advisory lock. Used to dedupe concurrent webhook
// deliveries of the same physical message before the existing dedup checks
// can run — those checks compare against the DB, but in a true race both
// invocations read an empty DB at the same instant and both proceed,
// resulting in two persisted user rows AND two AI reply rows. Holding this
// lock for the entire pipeline ensures only one delivery wins; the loser
// returns early and the AI never runs twice.
function messageLockKey(
  userId: string,
  phone: string,
  externalMessageId: string | undefined,
  text: string
): string {
  if (externalMessageId) return `extid:${externalMessageId}`;
  // Fallback: bucket by minute so a legitimate retry of the same text in
  // the next minute isn't blocked.
  const minute = Math.floor(Date.now() / 60_000);
  return `content:${userId}:${phone}:${minute}:${text.slice(0, 120)}`;
}

function messageLockId(key: string): string {
  const hash = createHash("sha256").update(`webhook_msg:${key}`).digest();
  const lo = hash.readUInt32BE(0) & 0x7fffffff;
  const hi = hash.readUInt32BE(4);
  return (BigInt(lo) * BigInt(0x100000000) + BigInt(hi)).toString();
}

async function tryAcquireMessageLock(key: string): Promise<boolean> {
  const id = messageLockId(key);
  const rows = await prisma.$queryRawUnsafe<{ acquired: boolean }[]>(
    `SELECT pg_try_advisory_lock(${id}::bigint) AS acquired`
  );
  return rows[0]?.acquired === true;
...
async function releaseMessageLock(key: string): Promise<void> {
  const id = messageLockId(key);
  await prisma.$queryRawUnsafe(`SELECT pg_advisory_unlock(${id}::bigint)`).catch(() => {});
}
...
export async function processIncomingMessage(params: {
  ...
  // ── Per-message lock (anti-race) ─────────────────────────
  // Acquired BEFORE any DB reads/writes. If two webhook deliveries for the
  // same inbound message arrive concurrently (Uazapi retry, queue replay,
  // duplicated push), only one wins this lock — the other exits without
  // saving anything or triggering the AI. Held for the full pipeline so
  // the losing delivery doesn't get a chance to race the user-msg insert.
  const msgLockKey = messageLockKey(userId, phone, externalMessageId, text);
  const msgLockAcquired = await tryAcquireMessageLock(msgLockKey);
  if (!msgLockAcquired) {
    log.info("webhook_concurrent_delivery_skip", { msgLockKey: msgLockKey.slice(0, 60) });
    return;
  }

  try {
    await processWithMessageLock();
  } finally {
    await releaseMessageLock(msgLockKey);
  }

  async function processWithMessageLock() {
    // ── Load AI config ────────────────────────────────────────
